/**
 * Telegram broadcast state machine.
 *
...
export type BroadcastState = "draft" | "scheduled" | "sending" | "sent" | "cancelled";
...
export type TransitionResult =
    | { ok: true; next: BroadcastState | null }
...
} satisfies Record<BroadcastState, Record<BroadcastAction, BroadcastState | null | "rejected">>);
 ...
 * as a silent no-op.
 */
export function transition(state: BroadcastState, action: BroadcastAction): TransitionResult {
...
export function allowedActions(state: BroadcastState): BroadcastAction[] {
    return (Object.keys(TRANSITIONS[state]) as BroadcastAction[]).filter(
